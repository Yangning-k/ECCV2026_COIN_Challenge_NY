...
\author{Ning Yang$^{1,2}$ \and Yan Huang$^{3,2,4}$}
\date{August 2026}
...
$^{3}$University of Chinese Academy of Sciences \quad
$^{4}$FiveAges}\\[0.45em]
\textit{Code:}
\url{https://github.com/Yangning-k/ECCV2026_COIN_Challenge_NY}\\[0.12em]
\textit{Weights:}
\url{https://huggingface.co/Njoker/CoIN_Challenge_NY}
\end{center}
...
choice (including a full Qwen3.6-27B replica that underperforms 32B
Mix-FT on holdout FR), ask/conclude ratio, and a rejected
chain-rebalancing branch.
\end{abstract}

\section{Task, Metrics, and Protocol}

The official protocol presents a natural-language target description and a
stream of candidate images. At each step the agent emits exactly one of
three actions~\cite{zorzi2026coinqa,taioli2025coin}: \emph{match} the
candidate to the description, \emph{reject} it, or \emph{ask} the oracle
one clarifying question about the hidden target image. We keep the
challenge output schema
\texttt{<motivation>}/\texttt{<score>}/\texttt{<question>} with
$\mathrm{score}\in\{0,1,2\}$, mapped onto the environment as:
score~2 $\mapsto$ match (\texttt{conclusion=True});
score~0 $\mapsto$ reject (\texttt{conclusion=False});
score~1 $\mapsto$ ask (\texttt{conclusion=None}, one question).
Official training data comprise 167 episodes (50 categories, 367 images)
and six description types, identified in the JSONL as
\texttt{category}, \texttt{color}, \texttt{context},
\texttt{color\_feature}, \texttt{color\_context}, and
\texttt{color\_context\_feature} (tables use color+feature, color+context,
and color+ctx+feat.).

\paragraph{Metrics.}
We use the names from QAsk-Nav~\cite{zorzi2026coinqa}. Let
$n_{\mathrm{succ}}$ be the number of correct decisions in an episode and
$n_{\mathrm{dist}}$ the number of candidates. The episode stops at the
first error, so the number of attempted decisions is
$n_{\mathrm{dec}}=n_{\mathrm{succ}}$ if
$n_{\mathrm{succ}}\ge n_{\mathrm{dist}}$, else $n_{\mathrm{succ}}+1$.
\textbf{SR} is the mean over episodes of $n_{\mathrm{succ}}/n_{\mathrm{dec}}$.
\textbf{FR} is the fraction of episodes with
$n_{\mathrm{succ}}\ge n_{\mathrm{dist}}$.
\textbf{NQ} is the mean over episodes of (questions)$/$(unique
observations), with observations grouped by image identity as in the
official eval dump. Ask rate is the fraction of episodes with at least
one question. We do \emph{not} use a global
$\sum$successes$/\sum$observations ratio: it weights long episodes and
can exceed 1 under pixel de-duplication. The paper also reports
wall-clock Time; we log it but do not select on it.
...
Qwen3-VL-32B-Instruct---not necessarily the hidden-test oracle.
Model selection never trains on a 47-episode holdout obtained by
splitting official episodes along shared-image connected components
(zero image leakage). That holdout is a subset of official \emph{train},
not the hidden test.

\paragraph{Naming.}
The frozen inference/data prompt is the \emph{Structured Attribute
Prompt} (SAP; code \texttt{our\_prompt\_v3}).
\emph{Raw decoding} (code \texttt{baseline}) executes the parsed score
with no post-hoc override.
The \emph{hybrid} policy (code \texttt{dedup\_category\_only}) applies
question-dedup and a forced decision only on \texttt{category}.
Fine-tuned 32B variants: Syn-FT (synthetic Stage~2 only; \texttt{v3d}),
Mix-FT (Stage~2 continued on official train-split pairs; \texttt{v3o}),
Chain-FT (rejected rebalancing; \texttt{v3p}), Full-FT (submitted;
Mix-FT recipe with holdout pairs; \texttt{v3f}).
...
(Table~\ref{tab:oracle}). $|\Delta\mathrm{SR}|\le 0.007$ on every type. Under this low-ask regime the questioner almost never
...
rather than submitting the V2 model.
...
\bottomrule
\end{tabular}
\caption{Zero-shot SAP, raw decoding, same 32B oracle, full official set.
Qwen3.6 asks less and scores slightly higher. Qwen3.6 was served with a
transformers backend; 32B with vLLM.}
\label{tab:q36zs}
\end{table}
...
matches inference. Each training example is a complete
(description, candidate image, optional dialogue, motivation, score,
question) tuple.
...
LoRA rank 16 ($39.8$M trainable parameters on 32B; targets
\texttt{q\_proj}/\texttt{k\_proj}/\texttt{v\_proj}/\texttt{o\_proj}), images
...
over content tokens of length $\geq 2$) or a per-observation budget of
two questions, then re-prompts once to decide from answers already
collected (fallback: reject). Applying this rule to \emph{all} types cuts NQ to 0.74 but
...
\begin{tabular}{lccc}
\toprule
Setting (holdout-47, six-type mean) & SR & FR & NQ/obs \\
\midrule
Zero-shot + SAP, raw decoding & 0.766 & 0.688 & 0.41 \\
Syn-FT + hybrid & \textbf{0.823} & 0.709 & 3.45 \\
Mix-FT + hybrid (selected) & 0.801 & \textbf{0.713} & 0.67 \\
Chain-FT + hybrid (not used) & 0.772 & 0.677 & 0.54 \\
\bottomrule
\end{tabular}
\caption{Leakage-free holdout, six types averaged. Zero-shot uses raw
decoding; fine-tunes use the hybrid policy. Mix-FT is selected by FR,
then SR, then NQ.}
\label{tab:holdout}
\end{table}
...
$0.739/0.653/0.40$). Mix-FT/Full-FT exist to keep that accuracy while holding NQ in a
range compatible with interactive evaluation.

\subsection{Replica on Qwen3.6-27B}
\label{sec:q36ft}
We repeat Stage~1, Stage~2 (5:1), and Mix-FT on Qwen3.6-27B with the
same files, SAP, hybrid policy, image size, and LoRA rank
(Table~\ref{tab:q36ft}). The 3.6 replica used learning rate
$5\times 10^{-5}$ for all stages (32B used $1\times 10^{-4}$ for
Stages~1--2). Native Qwen3.6 was stronger; after SFT both Syn-FT and
Mix-FT trail the 32B counterparts on FR and SR. Mix-FT NQ is
competitive (0.54 vs.\ 0.67) but cannot overturn the FR gap
($-6.4$pp). We therefore do not switch the submission.
...
(rank 16), SAP (\texttt{our\_prompt\_v3}), temperature 0, hybrid policy
(\texttt{dedup\_category\_only}: forced decision only on
\texttt{category}). Development oracle: Qwen3-VL-32B-Instruct.
Do not pass \texttt{--prompt-variant} or \texttt{--policy}: repository
defaults are already this configuration.
The Hugging Face repo provides two equivalent artifacts: merged bf16
weights under \texttt{merged/} ($\sim$63\,GB) or the LoRA under
\texttt{adapter/} ($\sim$168\,MB) served with vLLM \texttt{--enable-lora}
on the public Qwen3-VL-32B-Instruct base. Step-by-step hidden-test
commands are in the repository README.

\paragraph{Limitations.}
(i)~Holdout-47 numbers are leakage-free w.r.t.\ Mix-FT but are still
drawn from official train, not the hidden test.
(ii)~Full-FT absorbs those 47 episodes, so Mix-FT on holdout remains
the only clean selection number; Full-FT is reported via a first-60
sanity slice only.
(iii)~All development numbers use a local Qwen3-VL-32B oracle; the
organizers' hidden-test oracle may differ.
(iv)~Mix-FT trades some SR on context and color+feature for much lower
NQ and a higher category FR.

\section{Reproducibility Notes}
Connected-component episode IDs (\texttt{official\_split.json}),
Stage~2 manifests, and the exact SAP string live in the code directory.
Metrics are recomputed from dumped JSON with
\texttt{stats\_results.py}, which implements Section~1.
Continuation runs always start from Syn-FT, not from Mix-FT, so Mix-FT
and Full-FT are parallel forks rather than a chain.
